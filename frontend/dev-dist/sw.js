...
define(['./workbox-f8ba3ea9'], (function (workbox) { 'use strict';
  ...
  workbox.registerRoute(({
    url,
    request
  }) => request.method === "GET" && url.origin === "http://localhost:5000" && ["/api/products", "/api/events", "/api/workshops"].some(path => url.pathname.startsWith(path)), new workbox.NetworkFirst({
